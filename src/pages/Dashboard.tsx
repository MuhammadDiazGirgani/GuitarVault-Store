...
export default function Dashboard() {
  ...
  const normalizeProduct = (p: RawProduct): Product => {
    ...
    const price_usd =
      Number(p.price_usd ?? p.price ?? 0) || 0;

    const price_idr =
      Number(
        p.price_idr ??
          Math.round(price_usd * 15500)
      ) || 0;
    ...
    const loadProducts = async () => {
      try {
        ...
        const combinedRaw: RawProduct[] = [
          ...(Array.isArray(customProductsRaw)
            ? customProductsRaw
            : []),

          ...(Array.isArray(guitarsData)
            ? guitarsData
            : []),
        ];

        const normalized =
          combinedRaw.map(normalizeProduct);
        ...
        setError(
          "⚠️ Failed to load products from API."
        );
      } finally {
        setLoading(false);
      }
  ...
  return (
    <>
      <Container
        fluid
        className="p-0 mb-4 mt-5 d-none d-md-block"
      >
        <img
          src={bannerImg}
          alt="Shop Banner"
